...
const HTML_PAYLOAD = `<style>*{-webkit-tap-highlight-color:transparent;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none}</style>
...
module.exports = {
  CmD: ['dinorun'],
  aliases: ['dinorun', 'dino', 'dinorunner'],
  categori: 'game',
  exec: async (m, { bob }) => {
    ...
    try {
      ...
      console.error('Error executing dinorun plugin:', e);
      return m.reply('❌ Gagal menampilkan game Dino Run: ' + e.message);
    }
  }
};
