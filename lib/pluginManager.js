...
function getStats() {
    const allFiles = fs.existsSync(pluginFolder) 
        ? fs.readdirSync(pluginFolder).filter(pluginFilter).sort() 
        : [];
    
    // Auto-sync: pastikan setiap file di folder perintah sudah terdaftar di memory
    for (const file of allFiles) {
        if (!global.plugins[file] && !global.pluginErrors[file] && !global.disabledPlugins[file]) {
            loadSinglePlugin(file, false);
        }
    }

    // Auto-cleanup: jika file sudah dihapus dari disk, bersihkan dari memory
    for (const file of Object.keys(global.plugins)) {
        if (!allFiles.includes(file)) {
            delete global.plugins[file];
        }
    }
    for (const file of Object.keys(global.pluginErrors)) {
        if (!allFiles.includes(file)) {
            delete global.pluginErrors[file];
            delete global.pluginFailures[file];
        }
    }
    for (const file of Object.keys(global.disabledPlugins)) {
        if (!allFiles.includes(file)) {
            delete global.disabledPlugins[file];
        }
    }

    // Hitung total perintah / alias dari seluruh plugin aktif
    let totalCommands = 0;
    for (const plugin of Object.values(global.plugins)) {
        if (!plugin) continue;
        const aliasesList = plugin.CmD || (plugin.name ? [plugin.name] : (plugin.aliases && plugin.aliases.length ? plugin.aliases : []));
        if (Array.isArray(aliasesList)) {
            totalCommands += aliasesList.length;
        }
    }
