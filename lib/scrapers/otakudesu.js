...
async function search(title) {
  ...
  let { data } = await axios.get(`https://otakudesu.blog/?s=${encodeURIComponent(title)}&post_type=anime`, { httpsAgent: agent, headers: defaultHeaders })
...
async function get(id) {
  ...
  let { data, request } = await axios.get(`https://otakudesu.blog/anime/${cleanId}/`, { httpsAgent: agent, headers: defaultHeaders })
  if(request.res.responseUrl == "https://otakudesu.blog/") throw new Error("Anime not found!")
...
async function stream(id) {
  ...
  let { data, request } = await axios({
    url: `https://otakudesu.blog/episode/${cleanId}/`,
    httpsAgent: agent,
    headers: defaultHeaders
  })
  if(request.res.responseUrl == "https://otakudesu.blog/") throw new Error("Episode not found!")
...
async function download(id, batch = false) {
  ...
  let { data, request } = await axios({
    url: `https://otakudesu.blog/${batch ? "batch/" : "episode/"}${cleanId}/`,
    httpsAgent: agent,
    headers: defaultHeaders
  })
  if(request.res.responseUrl == "https://otakudesu.blog/") throw new Error("Episode not found!")
...
async function schedule() {
  let { data } = await axios("https://otakudesu.blog/jadwal-rilis/", { httpsAgent: agent, headers: defaultHeaders })
  let $ = cheerio.load(data)
  let result = []
...
async function ongoingAnime(page) {
  ...
  let url = "https://otakudesu.blog/ongoing-anime/"
...
async function genre(q, page) {
  ...
  let { data, request } = await axios("https://otakudesu.blog/genres/" + q + ((page && page > 1) ? "/page/" + page : ""), { httpsAgent: agent, headers: defaultHeaders })
  if(request.res.responseUrl == "https://otakudesu.blog/") throw new Error("Genre or page not found!")
  let $ = cheerio.load(data)
...
async function genreList() {
  let { data } = await axios("https://otakudesu.blog/genre-list/", { httpsAgent: agent, headers: defaultHeaders })
  let $ = cheerio.load(data)
  ...
  $("div.venser > ul.genres > li > a").each(function() {
    ...
    let link = href.startsWith("http") ? href : "https://otakudesu.blog" + href
